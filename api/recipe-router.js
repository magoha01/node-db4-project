...
router.get("/animals", (req, res, next) => {
  helpers
    .getAnimals() //INCLUDING SPECIES NAME
    .then((animals) => {
...
router.delete("/species/:species_id", (req, res, next) => {
  // it would be nice to have species_id validation midd
  helpers
    .deleteSpecies(req.params.species_id)
    .then((count) => {
      if (count > 0) {
        res.status(204).end();
      } else {
